	...
	$(document).ready(function () {
			...
			if ($('.backmate_theme_layout').length) {
				...
				this.body_container.appendTo($('.o_web_client')).then(function () {
					...
					ajax.jsonRpc('/get_theme_style', 'call', {}).then(function (data) {
						...
						}else if (discuss_chatter_style != 'style_1') {
						...
						if (backend_all_icon_style) {
							$('#font_awesome_icon_style').val(backend_all_icon_style);
						}
						...
						console.log(">>>>>>>>>>>>>>>>predefined_list_view_style.",predefined_list_view_style)
						// if (predefined_list_view_boolean) {
						// 	$('#predefined_list_view_boolean').val(predefined_list_view_boolean);
						// }
		...
		$('.o_web_client').on('click', '.save_color', function (ev) {
			if (color_change == false) {
				...
				var predefined_list_view_style = $("#predefined_list_view_style").val();
				...
				var discuss_chatter_style_image = $("#discuss_chatter_style_image").val();
				console.log("predefined_list_view_boolean",predefined_list_view_boolean)
				setTimeout(function () {
				...
				var discuss_chatter_style_image_form = $('#discuss_chatter_style_image')[0];
				var data5 = new FormData(discuss_chatter_style_image_form);
				$.ajax({
					type: "POST",
					enctype: 'multipart/form-data',
					url: "/api/upload/multi",
		...
		$('.o_web_client').on('click', "#predefined_list_view_boolean", function (ev) {
			var predefined_list_view_boolean = $("#predefined_list_view_boolean").prop("checked");
			if (predefined_list_view_boolean) {
				$('#predefined_list_view_boolean').attr('checked', true);
				$(".predefined_list_view_style").css("display", "table-row");
		...
		$('.o_web_client').on('change', "#icon_style", function (ev) {
			var icon_style = $('#icon_style').val();
			if (icon_style != 'dual_tone') {
				$(".sh_config_dt_label").css("display", "none");
				$("#dual_tone_icon_color_1").css("display", "none");
		...
		$('.o_web_client').on('change', "#discuss_chatter_style", function (ev) {
			var discuss_chatter_style = $('#discuss_chatter_style').val();
			if (discuss_chatter_style == 'style_1') {
				$(".discuss_chatter_background_image").css("display", "none");
				$("#discuss_chatter_style_image").css("display", "none");
