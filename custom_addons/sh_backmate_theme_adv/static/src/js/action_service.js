...
//	let shcontrollerStack = []
    ...
    function _executeActWindowAction(action, options) {
        ...
        // Softhealer code start
        if(enable_multi_tab){
            if(window.event && window.event.bubbles || window.event && window.event.which ||  $("#sh_owl_items div").length == 0 || action.xml_id =="crm.crm_lead_action_pipeline" || action.xml_id =="crm.crm_lead_action_forecast"){
