...
class RequestInformation(http.Controller):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'request_information_count' in counters:
            request_information_count = request.env['res.partner'].search_count([
                ('email', '=', post['email'])], limit=1)
            values['request_information_count'] = request_information_count
        return values
