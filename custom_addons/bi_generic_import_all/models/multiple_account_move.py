...
class gen_multiple_journal_entry(models.TransientModel):
	...
	def find_account_analytic_id(self, analytic_account_name):
		...
		if analytic_account_id:
			...
			raise ValidationError(_('"%s" Wrong Analytic Account Name %s') % analytic_account_name)
	...
	def create_import_move_lines(self, values):
		...
		if values.get('currency_id'):
			...
			if cur_name  != '' and cur_name != None:    
				...
				if currency_id != None :
					...
					raise ValidationError(_('"%s" Currency %s is not  in the system') % cur_name)
		...
		if values.get('account_id'):
			...
			if account_id != None:
				...
				raise ValidationError(_('"%s" Wrong Account Code %s') % account_code)
		...
		if values.get('analytic_account_id') != '':
			...
			if account_anlytic_account != '' or account_anlytic_account == None:
				...
				raise ValidationError(_('"%s" Wrong Account Code %s') % account_anlytic_account)	
