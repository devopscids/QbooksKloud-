    ...
    def get_custom_access_action(self):
        ...
        if website_id.domain:
            if website_id.domain.endswith('/'):
                ...
                url = domain_name + 'construction_email/feedback/' + str(self.id)
                return url
            else:
                domain = website_id.domain
                url = domain + '/construction_email/feedback/' + str(self.id)
                return url
        else:
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
            url = base_url + '/construction_email/feedback/' + str(self.id)
            return url
...
#                ctx.update({'search_default_my_tasks' : 0}) odoo13
    ...
    def show_task(self):
        for rec in self:
            ...
            ticket_task_id = self.env['project.task'].search([('construction_ticket_id', '=', rec.id)])
            task_ticket_list = list(dict.fromkeys(rec.custom_project_task_ids.ids + ticket_task_id.ids))
